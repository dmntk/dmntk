  ...
  Ok(Box::new(move |scope: &FeelScope| {
    ...
    match lhv {
      ...
      Value::Date(lh) => match rhv {
        ...
        other => {
          invalid_argument_type!("add", "years and months duration", other.type_of())
        }
      },
...
fn build_at(_bx: &BuildContext, text: &str) -> Result<Evaluator> {
  if let Ok(date) = FeelDate::from_str(text) {
    Ok(Box::new(move |_: &FeelScope| Value::Date(date.clone())))
  } else if let Ok(date_time) = FeelDateTime::try_from(text) {
    Ok(Box::new(move |_: &FeelScope| Value::DateTime(date_time.clone())))
  } else if let Ok(time) = text.parse::<FeelTime>() {
    Ok(Box::new(move |_: &FeelScope| Value::Time(time.clone())))
  } else if let Ok(ym_duration) = FeelYearsAndMonthsDuration::try_from(text) {
    Ok(Box::new(move |_: &FeelScope| Value::YearsAndMonthsDuration(ym_duration.clone())))
  } else if let Ok(dt_duration) = FeelDaysAndTimeDuration::try_from(text) {
    Ok(Box::new(move |_: &FeelScope| Value::DaysAndTimeDuration(dt_duration.clone())))
  } else {
    let invalid_at_literal = format!("invalid @ literal: {}", text);
    Ok(Box::new(move |_: &FeelScope| value_null!(invalid_at_literal)))
  }
}
...
/// ```
fn build_and(bx: &BuildContext, lhs: &AstNode, rhs: &AstNode) -> Result<Evaluator> {
  let lhe = build_evaluator(bx, lhs)?;
  let rhe = build_evaluator(bx, rhs)?;
  Ok(Box::new(move |scope: &FeelScope| {
    let lhv = lhe(scope);
...
fn build_every(bx: &BuildContext, lhs: &AstNode, rhs: &AstNode) -> Result<Evaluator> {
  ...
  let AstNode::Satisfies(satisfies) = rhs else {
    return Err(err_expected_ast_node("AstNode::Satisfies", &format!("{rhs:?}")));
  };
...
fn build_named_parameters(bx: &BuildContext, lhs: &[AstNode]) -> Result<Evaluator> {
  let mut evaluators = vec![];
  for item in lhs {
...
/// otherwise   otherwise    null
/// ```
fn build_or(bx: &BuildContext, lhs: &AstNode, rhs: &AstNode) -> Result<Evaluator> {
  let lhe = build_evaluator(bx, lhs)?;
  let rhe = build_evaluator(bx, rhs)?;
  Ok(Box::new(move |scope: &FeelScope| {
    let lhv = lhe(scope);
...
fn build_qualified_name_from_path(node: &AstNode) -> Result<QualifiedName> {
  match node {
    ...
    AstNode::Name(name) => Ok(name.clone().into()),
    _ => Err(err_unexpected_ast_node(&format!("expected Path or Name, found: {node:?}"))),
  }
}
