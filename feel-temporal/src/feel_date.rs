...
pub fn is_valid_date(year: Year, month: Month, day: Day) -> bool {
  if (-999_999_999..=999_999_999).contains(&year)
    && let Some(last_day_of_month) = last_day_of_month(year, month)
  {
...
mod tests {
  ...
  fn test_add_months() {
    ...
    assert_eq!("262142-02-01", format!("{}", FeelDate(262142, 1, 1).add_months(1).unwrap()));
  ...
  fn test_sub_months() {
    ...
    assert_eq!("-262142-01-01", format!("{}", FeelDate(-262142, 2, 1).sub_months(1).unwrap()));
